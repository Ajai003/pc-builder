...
class Title {
  ...
  constructor(
    { gl, plane, renderer, text, textColor = "#545050", font = "30px sans-serif" }: any,
    ogl: any
  ) {
    autoBind(this);
    this.gl = gl;
  ...
  createMesh(ogl: any) {
    const { Plane, Program, Mesh, Texture } = ogl;
...
class GalleryApp {
  ...
  constructor(
    ...
    try {
      ...
      console.warn('CircularGallery: WebGL context creation failed', e);
      this.destroyed = true;
      return;
    }

    if (!this.gl) {
      console.warn('CircularGallery: No WebGL context available');
      this.destroyed = true;
      return;
    }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    import("ogl").then((ogl) => {
      ...
      try {
        appRef.current = new GalleryApp(containerRef.current, ogl, {
          items,
          bend,
          textColor,
          borderRadius,
