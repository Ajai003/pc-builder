...
export function TestimonialsCard({
  ...
  return (
    <div className={cn("flex items-center justify-center p-8", className)}>
      ...
      >
        {showCounter && (
          <div className="col-start-2 row-start-1 text-right font-mono text-sm text-tech-silver/30">
          ...
          <AnimatePresence custom={direction}>
            {items.map((item, index) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className="absolute inset-0 w-full h-full overflow-hidden border-[3px] bg-tech-gray border-white/10 shadow-2xl rounded-xl"
                  initial={{
                    x: offset * 15,
                    y: Math.abs(offset) * 6,
        ...
        <div className="col-start-2 row-start-2 flex flex-col justify-center min-h-[120px]">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-sm text-tech-silver/50 mt-2 leading-relaxed">
        ...
        {showNavigation && items.length > 1 && (
          <div className="col-start-2 row-start-3 flex gap-2 mt-4">
            <button
              ...
              onClick={handlePrev}
              className={cn(
                "flex items-center justify-center w-10 h-10 rounded-full border border-white/10 bg-tech-gray transition-all",
                activeIndex === 0
                  ? "opacity-50 cursor-not-allowed"
                  : "hover:bg-white/10 hover:border-tech-cyan/30 hover:scale-105"
              )}
            >
              <ArrowLeft className="w-4 h-4 text-tech-silver/60" />
            </button>
            <button
            ...
            >
              <ArrowRight className="w-4 h-4 text-tech-silver/60" />
            </button>
          </div>
        )}
      </div>
